...
local function opt(zen, scope)
  ...
  if type(scope) == 'boolean' then scope = { global = scope, win = scope } end

  if scope.global then
    for _, global in ipairs(option.global) do
      local value = zen and global.disable_value or state[global.name] or not zen
...
local function zen(enter)
  ...
  if enter then
    ...
    if columns < textwidth or pad.width <= api.nvim_get_option('winwidth') then return end
    win.pad.create('left')
    ...
    if win.main.width >= win.main.buf.tw then return end
    win.pad.adjust()
    return
  end
...
function Zen.enter(args)
  ...
  for _, zen_opt in ipairs(option.zen) do
    ...
    if state.zen then
      ...
      if enter and not show[zen_opt.name] then value = zen_opt.disable_value end
...
function Zen.toggle(show)
  ...
  for _, zen_opt in ipairs(option.zen) do
    api.nvim_set_option(zen_opt.name, state.partial and zen_opt.disable_value or state[zen_opt.name])
  end
